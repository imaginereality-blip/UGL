...
public sealed partial class GamesConfigViewModel : ObservableObject
{
    ...
    /// field labels themselves and shown once, in a shared strip, instead of re-typed
    /// into every label ("System * (controller: Left/Right)", "BIOS Override (...; A to
    /// enter, Up/Down to move, A to remove, B to exit)"). The scheme is static per
    /// widget-type: every ComboBox/cycle field is always Left/Right, every reorderable
    /// list is always A/Up-Down/A/B, etc. Covers both the Settings and Art sub-tabs.
    /// </summary>
    public string CurrentFieldHint
    {
        get
        {
            if (IsArtTabActive)
            {
                return ArtFocusIndex switch
                {
                    6 => "A to toggle",
                    7 => "Left / Right to cycle scrape matches",
                    _ => string.Empty,
                };
            }

            return SettingsFocusIndex switch
            {
                1 or 3 or 5 => "Left / Right to change",
                2 => "A to enter, Up/Down to move, A to toggle, B to exit",
                6 => "A to toggle",
                13 => "A to enter, Up/Down to move, A to remove, B to exit",
                15 => "A to enter, Up/Down to move, A to toggle, B to exit",
                17 or 18 => "Left / Right to change",
                20 => "A to enter, Up/Down to move, A to remove, B to exit",
                _ => string.Empty,
            };
        }
    }
